...
const gameManager = createGameManager(io, undefined, undefined, 'whoami');
...
app.get('/healthz', (request, response) => {
  ...
  response.json({ ok: true, service: 'whoami', rooms: gameManager.getRoomCount() });
});
...
if (existsSync(path.join(clientDist, 'index.html'))) {
  ...
  app.get('/', (_request, response) => response.json({ service: 'whoami', healthz: '/healthz' }));
}
...
httpServer.listen(port, host, () => console.log(`[whoami] ouvindo em http://${host}:${port}`));
