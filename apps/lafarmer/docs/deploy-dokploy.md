...
1. Copie `apps/lafarmer/infra/.env.example` para `apps/lafarmer/infra/.env`.
2. Ajuste apenas os valores locais, se necessário.
3. A partir da raiz do repositório, valide a composição:

```text
docker compose --env-file apps/lafarmer/infra/.env -f apps/lafarmer/infra/docker-compose.dev.yml config
```

4. Suba o ambiente:

```text
docker compose --env-file apps/lafarmer/infra/.env -f apps/lafarmer/infra/docker-compose.dev.yml up --build
```
...
1. Crie uma aplicação Compose a partir do repositório e selecione `apps/lafarmer/infra/dokploy/docker-compose.yml` como arquivo.
2. Defina o diretório de build como a raiz do repositório. O `context: ../..` do Compose resolve a raiz do workspace em `apps/lafarmer/`.
